...
def trim(im, color):
    ...
    bbox = diff.getbbox()
    bbox = (bbox[0],bbox[1],bbox[2]+grid_size,bbox[3]+grid_size)
    if bbox:
        return im.crop(bbox)
...
def colors_do_match(el,target):
    return  colors_match(el[0],target[0]) and \
            colors_match(el[1],target[1]) and \
            colors_match(el[2],target[2])
...
with Image.open("image.png") as im:
    ...
    #                     # is_matching_wall = colors_do_match(pix[y][x],color_wall)
    #                     # is_matching_door = colors_do_match(pix[y][x],color_door)
    #                     # is_matching_window = colors_do_match(pix[y][x],color_window)
    #                     # if y < 3 and is_matching_wall:
    #                     #     wall_left = True
    #                     # if y > grid_size-3 and is_matching_wall:
    #                     #     wall_right = True
    #                     # if x < 3 and is_matching_wall:
    #                     #     wall_top = True
    #                     # if x > grid_size-3 and is_matching_wall:
    #                     #     wall_bottom = True
    #                     # if is_matching_door:
    #                     #     door = True
    #                     # if is_matching_window:
    #                     #     window = True
    ...
    for key in mastergrid:
        ...
        if item!=None:
            ...
            # if item["left"]:
                # print(item["left"],t,t in mastergrid,mastergrid[t])
            if item["left"] and t in mastergrid and mastergrid[t]==None:
                mastergrid[key]["left"] = None
                print("item gone")

            t = f"{int(x+grid_size)}:{int(y)}"
            if item["right"] and t in mastergrid and mastergrid[t]==None:
                mastergrid[key]["right"] = None
                print("item gone")

            t = f"{int(x)}:{int(y-grid_size)}"
            if item["top"] and t in mastergrid and mastergrid[t]==None:
                mastergrid[key]["top"] = None
                print("item gone")
            
            t = f"{int(x)}:{int(y+grid_size)}"
            if item["bottom"] and t in mastergrid and mastergrid[t]==None:
